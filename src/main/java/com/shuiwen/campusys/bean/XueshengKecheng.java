package com.shuiwen.campusys.bean;

public class XueshengKecheng {
	private int id;
	private int xueshengid;
	private String xingming;
	private int kechengid;
	private String bj_mingzi;
	private float xuefei;
	private String shoukuanren;
	private int queqinliang;
	private String qq_beizhu;
	private String pingjia;
	
	
	public int getId() {
		return id;
	}
	public void setId(int id) {
		this.id = id;
	}
	public int getXueshengid() {
		return xueshengid;
	}
	public void setXueshengid(int xueshengid) {
		this.xueshengid = xueshengid;
	}
	public int getKechengid() {
		return kechengid;
	}
	public void setKechengid(int kechengid) {
		this.kechengid = kechengid;
	}
	public float getXuefei() {
		return xuefei;
	}
	public void setXuefei(float xuefei) {
		this.xuefei = xuefei;
	}
	public String getShoukuanren() {
		return shoukuanren;
	}
	public void setShoukuanren(String shoukuanren) {
		this.shoukuanren = shoukuanren;
	}
	public int getQueqinliang() {
		return queqinliang;
	}
	public void setQueqinliang(int queqinliang) {
		this.queqinliang = queqinliang;
	}
	public String getQq_beizhu() {
		return qq_beizhu;
	}
	public void setQq_beizhu(String qq_beizhu) {
		this.qq_beizhu = qq_beizhu;
	}
	public String getPingjia() {
		return pingjia;
	}
	public void setPingjia(String pingjia) {
		this.pingjia = pingjia;
	}
	public String getXingming() {
		return xingming;
	}
	public void setXingming(String xingming) {
		this.xingming = xingming;
	}
	public String getBj_mingzi() {
		return bj_mingzi;
	}
	public void setBj_mingzi(String bj_mingzi) {
		this.bj_mingzi = bj_mingzi;
	}
	
	
	
}
